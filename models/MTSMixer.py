...
class FactorizedTemporalMixing(nn.Module):
    def __init__(self, input_dim, mlp_dim, match) :
        super().__init__()
    ...
    def match_loss(self, even, odd):
        return ((even - odd) ** 2).mean()
    ...
    def forward(self, x):
        ...
        if self.match:
            return x, self.match_loss(x_even, x_odd)
        else:
            return x
...
class MixerBlock(nn.Module):
    def __init__(self, tokens_dim, channels_dim, tokens_hidden_dim, channels_hidden_dim, fac_T, fac_C, match, norm_flag):
        super().__init__()
        self.tokens_mixing = FactorizedTemporalMixing(tokens_dim, tokens_hidden_dim, match) if fac_T else MLPBlock(tokens_dim, tokens_hidden_dim)
    ...
    def forward(self,x):
        ...
        if self.match:
            y, loss = self.tokens_mixing(y.transpose(1, 2))
            y = y.transpose(1, 2)
        else:
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.mlp_blocks = nn.ModuleList([
            MixerBlock(configs.seq_len, configs.enc_in, configs.d_model, configs.d_ff, configs.fac_T, configs.fac_C, configs.match, configs.norm) for _ in range(configs.e_layers)
        ])
