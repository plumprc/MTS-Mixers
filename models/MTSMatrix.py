...
class FactorizedTemporalMixing(nn.Module):
    def __init__(self, seq_len, enc_in, sampling, mat) :
        super().__init__()

        assert sampling in [1, 2, 3, 4, 6, 8, 12]
        self.sampling = sampling
        self.temporal_fac = nn.ModuleList([
            Matrix(seq_len // sampling, enc_in, mat) for _ in range(sampling)
        ])
...
class Model(nn.Module):
    def __init__(self, configs):
        super().__init__()
        self.matrix = FactorizedTemporalMixing(configs.seq_len, configs.enc_in, configs.sampling, configs.mat)
